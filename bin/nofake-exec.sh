...
if [ x"${output}" = x ]; then
    ...
    output_base=`temporary_file`
    tmpfiles="${tmpfiles:+${tmpfiles} }'${output_base}'"
    tmpfiles="${tmpfiles:+${tmpfiles} }'${output_base}.stamp'"
    ...
    for arg; do
        ...
        tmpfiles="${tmpfiles:+${tmpfiles} }'${output_base}${arg}'"
        tmpfiles="${tmpfiles:+${tmpfiles} }'${output_base}${arg}.stamp'"
        if [ x"${suffix}" != x ]; then
            # also consider these for good measure
            tmpfiles="${tmpfiles:+${tmpfiles} }'${output_base}${suffix}${arg}'"
            tmpfiles="${tmpfiles:+${tmpfiles} }'${output_base}${suffix}${arg}.stamp'"
        fi
    done

    # now the real output with or without suffix
    if [ x"${suffix}" != x ]; then
        output=${output_base}${suffix}
        tmpfiles="${tmpfiles:+${tmpfiles} }'${output}'"
        tmpfiles="${tmpfiles:+${tmpfiles} }'${output}.stamp'"
    else
        output=${output_base}
        # already scheduled for deletion in 'tmpfiles'
    fi
fi
